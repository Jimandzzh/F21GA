...
bool rotateModel = false;							//for mouse clicky
...
void startup()
{
	...
	content.LoadGLTF("assets/pendant24.gltf");
...
void render()
{
	...
	// Clear colour buffer
	glm::vec4 notgrayLilac = glm::vec4(0.831f, 0.5f, 0.9f, 1.0f);
	glm::vec4 backgroundColor = notgrayLilac;
	...
	modelMatrix = glm::scale(modelMatrix, glm::vec3(1.1f, 1.1f, 1.1f));//made changes here
...
bool isPointInsideModel(float x, float y)
{
...
void onMouseButtonCallback(GLFWwindow *window, int button, int action, int mods)
{
	if(button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
	{
		...
		float normalizedX = static_cast<float>(xpos) / screenWidth * 2.0f - 1.0f;
		float normalizedY = 1.0f - static_cast<float>(ypos) / screenHeight * 2.0f;

		if(isPointInsideModel(normalizedX, normalizedY))
		{
			rotateModel = true;
		}else
		{
